...
@section('content-header')
    <h1>
        {{ trans('shop::currencies.title.edit currency') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.shop.currency.index') }}">{{ trans('shop::currencies.title.currencies') }}</a></li>
        <li class="active">{{ trans('shop::currencies.title.edit currency') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.shop.currency.update', $currency->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                                ...
                                <select class="selectize" name="code" id="code">
                                    @foreach (Currency::getCurrencies() as $code => $item)
                                        <option value="{{ $code }}" {{ $code == old('code', $currency->code)  ? 'selected' : '' }}>
